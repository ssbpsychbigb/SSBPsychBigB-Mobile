...
export type EmailVerificationOtpResult = {
  alreadyVerified?: boolean;
  user: AuthUser;
};
...
export const authApi = {
  ...
  sendEmailVerification(token: string): Promise<EmailVerificationSendResult> {
    return apiRequest<EmailVerificationSendResult>(
  ...
  verifyEmailOtp(
    ...
    return apiRequest<EmailVerificationOtpResult>('/auth/email/verify-otp', {
      method: 'POST',
      token,
      body: { otp },
    });
  ...
  verifyEmailToken(token: string): Promise<EmailVerificationTokenResult> {
    return apiRequest<EmailVerificationTokenResult>(
  ...
  declineCollaboration(
    ...
    profileId: string,
    input?: { reason?: string },
  ): Promise<EducatorProfileSummary | { id: string; deleted: boolean }> {
    return apiRequest<EducatorProfileSummary | { id: string; deleted: boolean }>(
